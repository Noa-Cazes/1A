...
package body Arbre_Genealogique is 
    ...
    begin
        if (Is_Empty (Ab)) then
            ...
            Display_Title_Set ("L'ensemble des indivDATAus qui ont les deux parents connus", -1);
            To_String (E);
            Detruire (E);
        end if;
    ...
    function Ancetres_Homonymes (Ab1, Ab2 : in T_ABG; R : in T_Registre; ID1, ID2 : in Integer) return Boolean is
        ...
        procedure Compare_DATA_Tree (ID : in Integer; Ab2 : in T_ABG; R : in T_Registre; Res : out Boolean) is
        begin
            if (Is_Empty (Ab2)) then
                ...
                if (Get_Last_Name (La_Donnee_R (R, ID)) = Get_Last_Name (La_Donnee_R (R, Get_ID (Ab2))) and 
                    ...
                    Compare_Data_Tree (ID, Get_Left (Ab2), R, Res);
                    Compare_Data_Tree (ID, Get_Right (Ab2), R, Res);
                end if;
            end if;
        ...
        begin
            if (Is_Empty (Ab1)) then
                ...
                Compare_Data_Tree (Get_ID (Ab1), Ab2, R, Res);
