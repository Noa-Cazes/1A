...
package Arbre_Binaire is
    ...
    -- Nom : Get_ID
    -- Sémantique : Obtenir l'ID de la racine de Tree.
    -- Type_De_Retour : T_ID est l'ID de la racine de Tree.
    -- Paramètres
    ...
    -- Nom : Set_ID
    -- Sémantique : Modifier l'ID de la racine de Tree.
    -- Paramètres : 
    --    Tree      -- Tree que l'on va parcourir.
    --    ID        -- L'ID que l'on va mettre dans Tree.all.ID.
    ...
    -- Paramètre :
    --     Tree                  -- L'arbre pour lequel on cherche à savoir s'il est vide.
    ...
    -- Paramètres : 
    --     Tree      -- Tree que l'on va parcourir.
    --     ID        -- L'ID pour lequel on voudra savoir s'il est dans Tree.
    ...
    --     ID         -- L'ID que l'on va placer dans Node.all.ID. 
    procedure Create_Node (Node :in out T_BT; ID : in T_ID);

    -- Nom : Insert
    -- Sémantique : Insérer un ID associé à un nouveau noeud dans Tree.
    -- Exception  : Present_ID_Exception si l'ID est déjà dans Tree.
    -- Paramètres :
    --     Tree     -- Tree où on veut insérer un noeud.
    --     ID       -- L'ID correspondant au noeud que l'on veut insérer.
    generic
        -- Nom : gt
        -- Sémantique : Vérifie si ID1 est supérieur à ID2.
        -- Type_De_Retour : Boolean -- True si ID1 > ID2 ,sinon False.
        -- Paramètres
        --     ID 1|2   : Les IDs que l'on voudra comparer.
    ...
    -- Nom : Display
    -- Sémantique : Afficher un Tree dans l'ordre croissant des IDs.
    -- Paramètre
    --     Tree    -- Tree à afficher.
    generic
        -- Nom : Display_ID
        -- Sémantique : Afficher un ID passé en paramètre.
        -- Paramètre : 
        --     ID    -- ID à afficher.
