...
procedure Exemples_Memoire_Dynamique is
	...
	begin
		...
		Ptr1.all := 5;
		Put_Line ("Ptr1.all = " & Integer'Image (Ptr1.all));
		Free(Ptr1);  -- élément qu'il fallait rajouter pour enlever erreur de l'ex 2 sur valkyrie.
	...
	procedure Illustrer_Pile_Locale is
		...
		P : T_Pile;     -- dans le package Pile -> type T_Pile is access T_Cellule
	begin
		Initialiser (P);
		Empiler (P, 4);
		...
		Empiler (P, 2);
		Put_Line ("Sommet = " & Integer'Image (Sommet (P)));
                Detruire (P);  -- élémnt qu'il fallait rajouter pour enlever l'erreur de l'ex 5. 
		-- XXX P étant une variable locale du sous-programme, a-t-on besoin
		-- d'appeler Detruire dessus ? Oui, car on a libéré la mémoire dynamique utilisée automatiquement car P est une varibale locale. Mais la pile est tjrs là, d'où la necessité de faire Detruire(P). 
