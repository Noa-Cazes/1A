...
/* 1 pipe depuis chaque fils vers le père, lecture alternée */
...
int main() {
    ...
    do {
	/* nombre de messages reçus à chaque passafe */
	nb_recus = 0;
	/* lire ce qu eles fils envoient un par un */
	for (fils = 1; fils <= NB_FILS; fils++) {
	    if((read(pipe_f2p[fils][0], &pid, sizeof(int))) >0 ) {
