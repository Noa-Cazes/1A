...
int main() {
	...
	if (retour < 0) {
	    ...
	    write(pipe_f2p[1], message, strlen(message));
	    
	    sleep(2 * (NB_FILS - fils)); // orde décroissant 
	    bzero(message, sizeof(message));
	    sprintf(message, "Au revoir de la part du fils %d\n", fils);
	    write(pipe_f2p[1], message, strlen(message));
    ...
    while((read(pipe_f2p[0], message, sizeof(message)) > 0)) {
	printf("\nProcessus Pricipal - reçu: %s\n", message);
	bzero(message, sizeof(message));
    }
