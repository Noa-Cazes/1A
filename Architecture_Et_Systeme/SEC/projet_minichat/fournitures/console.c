...
int main (int argc, char *argv[]) {
    ...
    /* ouverture du tube d'écoute (déjà créé par le serveur */
    ecoute = open("./ecoute",O_WRONLY);
    if (ecoute==-1) {
        printf("Le serveur doit être lancé, et depuis le même répertoire que le client\n");
        exit(2);
    }
    /* Création des tubes de service (à faire) */
    /* Def du pseudo */
    strcpy(pseudo, argv[1]); 
    /* Donner le nom aux tubes */
    sprintf(tubeC2S, "%s_C2S", pseudo); 
    sprintf(tubeS2C, "%s_S2C", pseudo); 
    /* Créer les tubes (fifo) */
    ...
    if (strcmp(pseudo,"fin")!=0) { /* "console fin" provoque la terminaison du serveur */
       ...
       C2S = open(tubeC2S, O_CREAT | O_TRUNC | O_WRONLY, 0640);
       S2C = open(tubeS2C, O_CREAT | O_TRUNC | O_RDONLY, 0640);
        while (strcmp(saisie,"au revoir")!=0) {
            /* récupérer les messages reçus éventuels */
            FD_ZERO(&readfds);
            FD_SET(S2C, &readfds); 
            FD_SET(0, &readfds); 
            ...
            if (res > 0) {
                if (FD_ISSET(S2C, &readfds)) {
                    /* Vider le buffer message */
                    //bzero(message, TAILLE_MSG); 
                    /* Lire dans le tube S2C */
                    nlus = read(S2C, message, TAILLE_MSG);

                    if (nlus != 0) { 
                        /* Afficher les messages */
                        snprintf(discussion[curseur], TAILLE_MSG, "%s", message); 
                        afficher(curseur); 
                        curseur++;
                        curseur = curseur%NB_LIGNES; 
                   }
                } 
                if (FD_ISSET(0, &readfds)) {
                    /* Vider le buffer saisie */
                    //bzero(saisie, TAILLE_SAISIE); 
                    /* Récupérer la ligne saisie */
                    nlus = read(0, saisie, TAILLE_SAISIE);
                    if (nlus != 0) {
                        saisie[nlus-1] = '\0';
                        snprintf(message, TAILLE_MSG, "[%s] %s", pseudo, saisie); 
                        /* L'envoyer dans le tube C2S */
    ...
    /* Détruire le tube S2C */
    unlink(tubeS2C); 
    /* Détruire le tube C2S */
    unlink(tubeC2S);
    printf("fin client\n");
    exit (0);
