...
for i=[1:nb_lignes]
    for j = [2 : nb_col]
        I_decorrelee(i,j) = I(i,j) - I(i,j-1);
    end;
end;    
...
histogramme = hist(I_decorrelee(:),[I_decorrelee_min:I_decorrelee_max]);
figure('Name','Histogramme des niveaux de gris','Position',[0.33*L,0,0.33*L,0.3*L]);
plot([I_decorrelee_min:I_decorrelee_max],histogramme);
axis([I_decorrelee_min I_decorrelee_max 0 1.1*max(histogramme)]);
...
plot(X,Y,'b+');
axis([I_decorrelee_min I_decorrelee_max I_decorrelee_min I_decorrelee_max]);
...
x = I_decorrelee_min:I_decorrelee_max;
y = a*x+b;
