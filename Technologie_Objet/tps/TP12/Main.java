...
public class Main {
	...
	private static void repondreQuestions(Reader in) {
		Iterable<PointDeVente> iterable = PointDeVenteUtils.fromXML(in);  // in nom du fichier qui contient les prix
        ...
        while (iterable.iterator().hasNext()) {
        	pdvs.put(key, iterable.);
        	iterable.iterator() = iterable.iterator().next();
        	key ++;
        }
