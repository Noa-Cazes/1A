...
% Boucle
while continuer
    grad = grad_MC_CD(beta);
    Hess = Hess_MC_CD(beta);
    delta = -grad/Hess;
    
    
    if norm(grad(beta+delta)) <= max(Tol*norm(grad(beta)), eps)
        continuer = false;
        ifail = 1;
    elseif norm(delta) <= max(Tol*norm(beta), eps)
        continuer = false;
        ifail = 2;
...
%%%%%%%%%% TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1- Minimisation de la fonction des moindres carres par la m�thode 
%    de Gauss-Newton
