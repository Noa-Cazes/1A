...
r_R_V = sqrt(sigma(2,1)/(sigma(1,1) * sigma(2,2)));
r_R_B = sqrt(sigma(3,1)/(sigma(1,1) * sigma(3,3)));
r_B_V = sqrt(sigma(3,2)/(sigma(3,3) * sigma(2,2)));
...
%%  Dans un nouveau repère
% Changement de base
[W,D] = eig(sigma);
[D,Ind] = sort(diag(D),"descend");
W = W(:,Ind);
C = X_c * W;
%C = X_c * W * W' + un_v * (moy_x)';
N = size(I);
im = reshape(C, N);
...
% Coefficients de corrélation linéaire
r_R_V = sqrt(sigman(2,1)/(sigman(1,1) * sigman(2,2)));
r_R_B = sqrt(sigman(3,1)/(sigman(1,1) * sigman(3,3)));
r_B_V = sqrt(sigman(3,2)/(sigman(3,3) * sigman(2,2)));

% Proportions de contraste
c_R = sigman(1,1) / (sigman(1,1) + sigman(2,2) + sigman(3,3));
c_V = sigman(2,2) / (sigman(1,1) + sigman(2,2) + sigman(3,3));
c_B = sigman(3,3) / (sigman(1,1) + sigman(2,2) + sigman(3,3));
...
% Affichage de la première combinaison linéaire
    subplot(2,2,3) % 2ème ligne, 1ère colonne
    imagesc((1/3)*(R + V + B))
    axis off image
    title('première combinaison linéaire','FontSize',20)
    
% Affichage de la deuxième combinaison linéaire
    subplot(2,2,4) % 2ème ligne, 2ème colonne
    imagesc(0.2989 * R + 0.5870 * V + 0.1140 * B)
    axis off image
    title('deuxième combinaison linéaire','FontSize',20)
