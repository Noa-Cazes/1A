...
for i = 1:k
  ...
  % Perte d'orthogonalite avec algorithme classique
  Qcgs1 = cgs(cgs(A));
  po(3,i) = norm(eye(n)-Qcgs1'*Qcgs1);
  
  % Perte d'orthogonalite avec algorithme modifie
  Qmgs1 = mgs(mgs(A));
  po(4,i) = norm(eye(n)-Qmgs1'*Qmgs1);
...
figure('Position',[0.1*L,0.1*H,0.8*L,0.75*H])
    ...
    grid on
    loglog(x,po(3,:),'v','lineWidth',2)
    hold on
    loglog(x,po(4,:),'p','lineWidth',2)
    grid on
    leg = legend('Gram-Schmidt classique 1',...
                 'Gram-Schmidt modifie 1',...
