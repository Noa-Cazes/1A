function [TEB] = Ch16QAM(alpha, Rb, Eb_N0_dB, display)
%Ch16QAM Cha�ne de transmission avec modulation 16-QAM
...
%       g�n�r� et le message binaire obtenu
    ...
    symbolesG = QAMMod(binMsg, M, (ismember('all', display) | ismember('sym', display)) - 1)';
    ...
    for i = 1:length(Eb_N0)
        ...
        msgR = QAMDemod(signalE, M, (ismember('all', display) | ismember('res', display)) - 1);

        %% TEB

        % TES = 1 - sum(symbolesG == symbolesD) / length(symbolesG);
        TEB(i) = 1 - sum(msgR == binMsg) / nbBits;
        TEB_th(1,i) = qfunc(sqrt(Eb_N0(1,i)))/2;
    end
    ...
    if(ismember('all', display) || ismember('teb', display))
        ...
        title('Evolution du Taux d''Erreur Binaire fonction de (E_b/N_0)dB 16-QAM');
        ylabel('TEB');
        xlabel('(E_b/N_0)dB');
    end
    
     %% CONSTELLATION
    constellation(binMsg, signalE, '16-QAM'); 
