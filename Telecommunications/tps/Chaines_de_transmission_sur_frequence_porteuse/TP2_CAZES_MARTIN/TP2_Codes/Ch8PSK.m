function [TEB] = Ch8PSK(alpha, Rb, Eb_N0_dB, display)
%Ch8PSK Cha�ne de transmission avec modulation 8-PSK
...
%       g�n�r� et le message binaire obtenu
    ...
    for i = 1:length(Eb_N0)
        ...
        signalF = shaping(signalR, h, (ismember('all', display) | ismember('f', display)) - 1);
    ...
    if(ismember('all', display) || ismember('teb', display))
        ...
        title('Evolution du Taux d''Erreur Binaire fonction de (E_b/N_0)dB 8-PSK');
        ylabel('TEB');
        xlabel('(E_b/N_0)dB');
    end
    
     %% CONSTELLATION
    constellation(bimsg2demsg(binMsg, M), signalE, '8-PSK'); 
end
