function [TEB] = Ch1(alpha, Rb, Eb_N0_dB, display)
%Ch1 Cha�ne de transmission de la partie 1
...
%       g�n�r� et le message binaire obtenu
    ...
    for i = 1:length(Eb_N0)
        ...
        signalC = addNoise(signalFP, Eb_N0(i), Ns, M, (ismember('all', display) | ismember('c', display)) - 1);
    ...
    if(ismember('all', display) || ismember('teb', display))
        ...
        title('Evolution du Taux d''Erreur Binaire foncion de (E_b/N_0)dB 4-ASK');
        ylabel('TEB');
        xlabel('(E_b/N_0)dB');
    end
