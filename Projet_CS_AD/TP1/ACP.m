...
X = X + 5*ones(nb_indiv,2); % DÃ©calÃ© de 5 Ã  droite
...
p2 = plot([X_m(1), X_m(1)+axes(1, 1)*10*Contraste(1)], [X_m(2), X_m(2)+axes(2, 1)*10*Contraste(1)],'k-',...
          [X_m(1), X_m(1)+axes(1, 2)*10*Contraste(2)], [X_m(2), X_m(2)+axes(2, 2)*10*Contraste(2)],'b-','linewidth',2);grid on; % /!\ L'affichage du point (0,0) ne sert a rien ici
% a part a montrer comment creer une legende qui reunit les courbes de deux
% plots differents (cf lignes de code 58-59)
title('Changement de repère : repère canonique VS repère principal')
legend([p1;p2],'Nuage de points','Origine','Premier axe canonique','Deuxième axe canonique',...
    'Individu moyen','Premier axe principal','Deuxième axe principal');
...
p3 = plot(X_c(:, 1), X_c(:, 2), "r+");grid on;axis equal;hold on;
p4 = plot(C(:, 1), C(:, 2), "b+");
legend('Individus projetés sur les axes canoniques', 'Individus projetés sur les axes principaux')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
info_axe_1 = Contraste(1)*100;
info_axe_2 = Contraste(2)*100;
info_axe_3 = sum(Contraste(1:2))*100;
