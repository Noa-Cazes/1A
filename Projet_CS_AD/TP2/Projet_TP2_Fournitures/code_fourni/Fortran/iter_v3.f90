...
!   with Rayleigh-Ritz projection + block approach (v3) + deflation () (TO BE DEVELOPPED)
!
!--------------------------------------------------------------------------------------------------------------------
!
!! subspace_iter_v3
...
! using the subspace iteration method with Rayleigh-Ritz projection + block approach (v3) + deflation ()
!
! ..Input arguments
...
!-------------------------------------------------------------------------------------------------------------------
  subroutine subspace_iter_v3(n, m, a, p, percentage, maxit, eps, v, w, n_ev, acc_ev, it_ev, ierr)
    ...
    !! local variables                                
    integer                                          :: i, j
    double precision, allocatable, dimension(:,:)    :: v_nc
    ...
    do while((eig_sum .lt. p_trace) .and. (n_ev .lt. m) .and. (k .lt. maxit))
      ...
      deallocate(y)
      allocate(v_nc(n, m-n_ev), y(n, m-n_ev))
      v_nc = v(:, n_ev+1:)
      do i = 1, p
        call dgemm('n', 'n', n, m, n, done, a, n, v_nc, n, dzero, y, n)
        v_nc = y
      end do
      v(:, n_ev+1:) = v_nc
      deallocate(v_nc)
      ...
      call gram_schmidt_block(y, n, m, v)
      !!write(*, '("LE PROBLEME EST ICI")')
...
999 continue
    deallocate(y, h, x, w_aux, aux_acc, t, v_nc)
    write(*,*)
